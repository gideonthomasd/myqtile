...
keys = [
    ...
    Key([mod], "x", lazy.spawn(home + "/dotfiles/scripts/powermenu.sh"), desc="Open Powermenu"),
    Key([mod], "d", lazy.spawn(home + "/dotfiles/scripts/applauncher.sh"), desc="Launch Rofi"),
    Key([mod], "r", lazy.spawn("alacritty -e ranger"), desc="Launch ranger"),
    #Key([mod], "F12", lazy.spawn(thunar), desc="Launch thunar"),

    # Apps
    Key([mod], "Return", lazy.spawn(terminal), desc="Launch terminal"),
    Key([mod, "control"], "Return", lazy.spawn(manager), desc="Launch Thunar"),
    ...
    Key([mod, "control"], "w", lazy.spawn("nitrogen"), desc="Select Theme and Wallpaper"),
...
layouts = [
    ...
    # layout.Tile(),
    layout.TreeTab(panel_width=75, bg_color="24283b"),
    # layout.VerticalTile(),
    # layout.Zoomy(),
    layout.Floating()
]
...
widget_list = [
     ...
     widget.TextBox(
                       ...
                       padding = 0,
                       fontsize = 25,
		       #mouse_callbacks={'Button3': lambda: qtile.cmd_spawn("lxterminal -e mocp")}
                       ),
     
     widget.Wttr(
		location = {'Caerphilly':'Home'},
		format = "1",
		font = "JetBrainsMono Nerd Font",
		fontsize=19,
		foreground='#FFFF99',
		mouse_callbacks={'Button1': lambda: qtile.cmd_spawn("alacritty -e bash -c 'curl wttr.in/Caerphilly && read'")}
		),          
...
if (wm_bar == "polybar"):
    screens = [Screen(top=bar.Gap(size=48))]
else:
    screens = [
        Screen(
            top=bar.Bar(
